...
namespace BTS.Web.Areas.Controllers
{
    ...
    public class WardController : BaseController
    {
        ...
        public ActionResult Add(WardVM Item)
        {
            try
            {
                if (ModelState.IsValid)
                {
                    ...
                    _WardService.Save();
                    return Json(new { resetUrl = Url.Action("Add", "Ward"), status = CommonConstants.Status_Success, html = GlobalClass.RenderRazorViewToString(this, "ViewAll", Mapper.Map<IEnumerable<WardVM>>(GetAll())), message = "Thêm dữ liệu thành công" }, JsonRequestBehavior.AllowGet);
                }
                else
                {
                    return Json(new { resetUrl = Url.Action("Add", "Ward"), status = CommonConstants.Status_Error, message = ModelState.Values.SelectMany(v => v.Errors).Take(1).Select(x => x.ErrorMessage) }, JsonRequestBehavior.AllowGet);
                }
            }
            catch (Exception ex)
            {
                return Json(new { resetUrl = Url.Action("Add", "Ward"), status = CommonConstants.Status_Error, message = ex.Message }, JsonRequestBehavior.AllowGet);
            }
        }
        ...
        public ActionResult AddOrEdit(string act, WardVM ItemVm)
        {
            try
            {
                if (ModelState.IsValid)
                {
                    ...
                    {
                        Ward editItem = _WardService.getByID(ItemVm.Id);
                        editItem.UpdateWard(ItemVm);
                        editItem.UpdatedBy = User.Identity.Name;
                        editItem.UpdatedDate = DateTime.Now;
        ...
        public ActionResult Delete(string id = "0")
        {
            try
            {
                ...
                {
                    return Json(new { resetUrl = Url.Action("Add", "Ward"), status = CommonConstants.Status_Error, message = "Không thể xóa Trường hợp kiểm định này do đã được sử dụnd" }, JsonRequestBehavior.AllowGet);
                }

                _WardService.Delete(id);
                _WardService.Save();

                return Json(new { resetUrl = Url.Action("Add", "Ward"), status = CommonConstants.Status_Success, html = GlobalClass.RenderRazorViewToString(this, "ViewAll", GetAll()), message = "Xóa dữ liệu thành công" }, JsonRequestBehavior.AllowGet);
            }
            catch (Exception ex)
            {
                return Json(new { resetUrl = Url.Action("Add", "Ward"), status = CommonConstants.Status_Error, message = ex.Message }, JsonRequestBehavior.AllowGet);
