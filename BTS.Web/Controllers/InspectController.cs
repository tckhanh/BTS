...
namespace BTS.Web.Areas.Controllers
{
    ...
    public class InspectController : BaseController
    {
        ...
        public InspectController(ICertificateService certificateService, IOperatorService operatorService, IProfileService profileService, ICityService cityService, IInCaseOfService inCaseOfService, ILabService labService, IErrorService errorService) : base(errorService)
        {
        ...
        public JsonResult loadCertificate()
        {
            ...
            if (CertNumStart > 0)
            {
                Items = Items.Where(x => Int32.Parse(x.Id.Substring(1,5)) >= CertNumStart) ;
            }

            if (CertNumEnd > 0)
            {
                Items = Items.Where(x => Int32.Parse(x.Id.Substring(1, 5)) <= CertNumEnd);
            }
