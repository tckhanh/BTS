...
var checkController = {
    ...
    registerEvent1: function () {
        ...
        $('#btnAddNew').off('click').on('click', function () {
            ...
            checkController.resetForm();
        });

        $('#btnSave').off('click').on('click', function () {
            if ($('#frmSaveData').valid()) {
                checkController.saveData();
            }
        });
        ...
        if (userRoleAdmin) {
            $("#MyDataTable")
                .on('xhr.dt', function (e, settings, json, xhr) {
                    ...
                    checkController.loadMap(json.data);
                    checkController.loadPivotTable(json.data);
                })
                .dataTable({
                    dom: 'Bfrtip',
                    buttons: [
