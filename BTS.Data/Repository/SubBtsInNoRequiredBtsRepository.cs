...
{
    public interface ISubBtsInNoRequiredBtsRepository : IRepository<SubBtsInNoRequiredBts>
    {
        IEnumerable<NoRequiredBts> GetMultiPagingByBtsCode(string btsCode, out int totalRow, int pageIndex = 1, int pageSize = 10, bool onlyOwner = false);

        IEnumerable<NoRequiredBts> GetMultiByBtsCode(string btsCode, bool onlyOwner = false);
    ...
    public class SubBtsInNoRequiredBtsRepository : RepositoryBase<SubBtsInNoRequiredBts>, ISubBtsInNoRequiredBtsRepository
    {
        public SubBtsInNoRequiredBtsRepository(IDbFactory dbFactory) : base(dbFactory)
        {
        }

        public IEnumerable<NoRequiredBts> GetMultiByBtsCode(string btsCode, bool onlyOwner = true)
        {
            btsCode = btsCode.Trim().ToUpper();
            if (onlyOwner)
            {
                IQueryable<NoRequiredBts> query = from noRequiredBts in DbContext.NoRequiredBtss
                                                join subBts in DbContext.SubBtsInNoRequiredBtss
                                                on noRequiredBts.Id equals subBts.NoRequiredBtsID
                                                where subBts.BtsCode.Trim().ToUpper() == btsCode && subBts.OperatorID == noRequiredBts.OperatorID
                                                orderby noRequiredBts.AnnouncedDate descending
                                                select noRequiredBts;
                return query;
            }
            else
            {
        ...
        public IEnumerable<NoRequiredBts> GetMultiPagingByBtsCode(string btsCode, out int totalRow, int pageIndex = 1, int pageSize = 10, bool onlyOwner = true)
        {
            btsCode = btsCode.Trim().ToUpper();
            if (onlyOwner)
            {
